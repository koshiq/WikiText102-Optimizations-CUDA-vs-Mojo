...
def train():
    ...
    # Define a schedule for the profiler to warm up and then record a trace.
    prof_schedule = torch.profiler.schedule(wait=1, warmup=1, active=3, repeat=1)
    
    # The on_trace_ready handler saves the trace to a file.
    # This file can be opened in chrome://tracing or NVIDIA Nsight.
    trace_handler = torch.profiler.tensorboard_trace_handler('./logs/transformer_trace')
    ...
    with torch.profiler.profile(
        ...
        for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
            ...
            if batch % args.log_interval == 0 and batch > 0:
                ...
                total_loss = 0
                start_time = time.time()
            prof.step()  # Notify the profiler about the step completion
            if args.dry_run and batch > 5: # Run a few batches for profiling
                break
...
with open(args.save, 'rb') as f:
    # The model is loaded with a safe_globals context manager to allowlist the
    # custom model classes we trust. This is the recommended practice since
    # PyTorch 2.6, where `weights_only` defaults to True for security.
    safe_globals = [
        TransformerModel,
        PositionalEncoding,
        # Add standard PyTorch modules used by the models to the allowlist.
        # This is necessary because `weights_only=True` restricts which classes
        # can be unpickled for security reasons.
    ...
    if MOJO_AVAILABLE:
        safe_globals.append(MojoTransformerModel)
